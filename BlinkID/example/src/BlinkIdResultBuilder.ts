import { BlinkIdScanningResult } from "../../src/blinkIdResult"
import { AddressDetailedInfo, BarcodeResult, BarcodeData, DataMatchResult, DateResult, DependentInfo, DocumentClassInfo, DriverLicenseDetailedInfo, MrzResult, SingleSideScanningResult, StringResult, VizResult } from "../../src/types";
...
export class BlinkIdResultBuilder {
  static getIdResultString(result?: BlinkIdScanningResult): string {
    ...
    const resultString =
        ...
        this.buildResult("Additional name info", result.additionalNameInformation) +
        this.buildResult("Address", result.address) +
        this.buildResult("Additional address info", result.additionalAddressInformation) +
        this.buildResult("Document number", result.documentNumber) +
        this.buildResult("Additional document number", result.documentAdditionalNumber) +
...
static buildSubresults(subResults?: SingleSideScanningResult[]): string {
  ...
  static buildMrzResult(result?: MrzResult) {
    ...
    const resultString =
        ...
        this.buildStringResult("Sanitized document code", result.sanitizedDocumentCode) +
        this.buildStringResult("Sanitized document number", result.sanitizedDocumentNumber) +
        this.buildStringResult("Sanitized issuer", result.sanitizedIssuer) +
        this.buildStringResult("Sanitized nationality", result.sanitizedNationality) +
        this.buildStringResult("Sanitized Opt1", result.sanitizedOpt1) +
        this.buildStringResult("Sanitized Opt2", result.sanitizedOpt2) +
        this.buildStringResult("Document type", result.documentType?.toString());
    ...
    const resultString =
        this.buildStringResult("Additional name information", result.additionalNameInformation) +
        this.buildStringResult("Address", result.address) +
        this.buildStringResult("Document additional number", result.documentAdditionalNumber) +
        this.buildStringResult("Document number", result.documentNumber) +
    ...
    const resultString =
        ...
        this.buildResult("Additional optional address information", result.additionalOptionalAddressInformation) +
        this.buildResult("Additional personal ID number", result.additionalPersonalIdNumber) +
        this.buildResult("Document additional number", result.documentAdditionalNumber) +
        this.buildResult("Document optional additional number", result.documentOptionalAdditionalNumber) +
        ...
        this.buildResult("Residential status", result.residentialStatus) +
        this.buildResult("Sex", result.sex) +
        this.buildResult("Specific document validity", result.specificDocumentValidity) +
        this.buildResult("Sponsor", result.sponsor) +
    ...
    if (info == null) return "";
    return this.buildStringResult("Street", info.street) +
  ...
  static buildDriverLicenseInfo(info?: DriverLicenseDetailedInfo<string>) {
    if (info == null) return "";
    return this.buildStringResult("Restrictions", info.restrictions) +
...
static buildDateResult<T>(propertyName: string, result?: DateResult<T>): string {
  ...
  static buildDriverLicenceResult<T>(result?: DriverLicenseDetailedInfo<T>) {
    if (result == null || result == undefined) return "";
        ...
        return this.handleStringType("Restrictions", result.restrictions) ?? ""+
  ...
  static buildDataMatchResult(dataMatchresult?: DataMatchResult) {
    if (dataMatchresult == null) return "";
    ...
    if (dataMatchresult.states != null || dataMatchresult.states != undefined) {
        for (const field of dataMatchresult?.states) {
      dataMatchResultString += `${field.field}: ${field.state}\n`;
    }
    }
    return dataMatchResultString;
  }

  static buildDependentsInfoResult(dependentInfoResult?: DependentInfo[]): string {
    if (dependentInfoResult == null || dependentInfoResult == undefined) return "";
  ...
  static buildBarcodeData(barcodeDataResult?: BarcodeData): string {
    ...
    return this.buildStringResult("Barcode type", barcodeDataResult.barcodeType) +
        this.buildStringResult("Barcode string data", barcodeDataResult.stringData);
  }
